...
function displayRiwayat(searchTerm = '') {
  ...
  container.innerHTML = filtered.map(t => `
    ...
    <div class="riwayat-item ${t.voided ? 'voided-transaction' : ''}" style="${t.voided ? 'opacity:0.6; background:#ffe0e0;' : ''}">
      <div>${t.date ? new Date(t.date).toLocaleString() : 'Tanggal tidak tersedia'} - ${t.platform || '?'} (${t.invoiceNumber || '-'})</div>
      <div>Total: Rp ${(t.total || 0).toLocaleString()} | Profit: Rp ${(t.profit || 0).toLocaleString()}</div>
    ...
    const htmlContent = `<!DOCTYPE html>
      ...
      <body>
        ...
        <table><thead><tr><th>No</th><th>Invoice</th><th>Tgl/Jam</th><th>Platf</th><th class="right">Total</th></tr></thead><tbody>
          ${filtered.map((t, i) => `<tr><td style="text-align:center">${i+1}</td><td>${escapeHtml(t.invoiceNumber?.slice(-12) || '-')}</td><td>${t.date ? new Date(t.date).toLocaleString('id-ID', {day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}) : '-'}</td><td>${escapeHtml(t.platform?.slice(0,6) || '-')}</td><td class="right">${formatRupiah(t.total || 0).replace('Rp', '').trim()}</td></tr>`).join('')}
        </tbody></table>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('/sw.js').then(reg => {
        console.log('Service Worker registrasi sukses', reg);
  ...
  // Kode usang tombol platform statis sudah dihapus!
...
// ==================== FITUR MANAJEMEN PLATFORM AKTIF ====================
// Kode ini ditambahkan di akhir file. Tidak mengganggu fungsi lain.
(function() {
  if (window._platformManagementActive) return;
  window._platformManagementActive = true;

  const allPlatforms = ['Grabfood', 'Gofood', 'Shopeefood', 'TikTok', 'Dinein'];
  let activePlatforms = [];

  function loadActivePlatforms() {
    const saved = localStorage.getItem('dewepos_activePlatforms');
    activePlatforms = saved ? JSON.parse(saved) : [...allPlatforms];
    activePlatforms = activePlatforms.filter(p => allPlatforms.includes(p));
    if (activePlatforms.length === 0) activePlatforms = [...allPlatforms];
  }
  ...
  function saveActivePlatforms() {
    ...
    showStatusMessage('✅ Pengaturan platform disimpan!');
  }
  ...
  function renderPlatformChecklist() {
    ...
    allPlatforms.forEach(platform => {
      ...
      div.innerHTML = `
        <input type="checkbox" id="chk_${platform}" value="${platform}" ${checked ? 'checked' : ''}>
  ...
  function renderPlatformButtons() {
    ...
    if (!container) {
      ...
      if (parent) {
        ...
        console.warn('Tidak menemukan .platform-selector untuk tombol platform');
        return;
      }
    }
    // Sembunyikan tombol platform statis (jika masih ada)
    document.querySelectorAll('.platform-btn:not(#platformButtonsContainer .platform-btn)').forEach(btn => {
    ...
    activePlatforms.forEach(platform => {
      ...
      btn.onclick = (function(p) {
        return function() {
          ...
          selectedPlatform = p;
          if (typeof setupPaymentOptions === 'function') setupPaymentOptions();
  ...
  const originalUpdatePendapatanSummary = window.updatePendapatanSummary;
  if (originalUpdatePendapatanSummary) {
      ...
      setTimeout(() => {
        ...
        if (summaryDiv) {
          ...
          cards.forEach(card => {
            ...
            if (platform && !activePlatforms.includes(platform)) {
  ...
  if (originalShowPage) {
    window.showPage = function(page) {
      ...
      if (page === 'transaksi') setTimeout(renderPlatformButtons, 10);
      if (page === 'pendapatan' && window.updatePendapatanSummary) setTimeout(window.updatePendapatanSummary, 10);
    };
  }
  ...
  function init() {
    ...
    if (window.updatePendapatanSummary) window.updatePendapatanSummary();
  }
...
// ==================== PREVIEW CETAK ULANG STRUK ====================
// Buat modal preview cetak ulang jika belum ada
if (!document.getElementById('reprintPreviewModal')) {
  const modalDiv = document.createElement('div');
  modalDiv.id = 'reprintPreviewModal';
  modalDiv.className = 'modal';
  modalDiv.style.display = 'none';
  modalDiv.innerHTML = `
    <div class="modal-content" style="max-width:400px;">
      <div class="modal-header">
        <h3>🖨️ Preview Cetak Ulang Struk</h3>
        <button class="close-reprint-preview">&times;</button>
      </div>
      <div id="reprintStrukContent" style="padding:10px; max-height:70vh; overflow-y:auto;"></div>
      <div style="padding:10px; text-align:center;">
        <button id="confirmReprintBtn" style="background:#4361ee; color:white; border:none; border-radius:8px; padding:8px 20px; cursor:pointer;">Cetak Struk</button>
      </div>
    </div>
  `;
  document.body.appendChild(modalDiv);
  const closeBtn = modalDiv.querySelector('.close-reprint-preview');
...
// Fungsi untuk menampilkan preview cetak ulang
function showReprintPreview(transaction) {
  const modal = document.getElementById('reprintPreviewModal');
  const contentDiv = document.getElementById('reprintStrukContent');
  ...
  const strukHtml = `
    ...
    ${paymentDetail}<hr><div style="text-align:center;">Terima Kasih!<br>⭐ Rating & Review ⭐</div>
  `;
  contentDiv.innerHTML = strukHtml;
  modal.style.display = 'block';
  
  // Event untuk tombol cetak di modal (pastikan tidak double)
  const printBtn = document.getElementById('confirmReprintBtn');
  const newPrintBtn = printBtn.cloneNode(true);
  printBtn.parentNode.replaceChild(newPrintBtn, printBtn);
  newPrintBtn.onclick = () => {
    const printContent = contentDiv.innerHTML;
    const win = window.open('', '_blank');
    win.document.write(`
      <html>
        <head>
...
// Perbaiki fungsi displayRiwayat: ganti tombol cetak ulang menggunakan showReprintPreview
// Cari bagian dalam displayRiwayat() yang berisi:
// ${!t.voided ? `<button onclick="reprintStruk(...)">...</button>` : ''}
// Ganti dengan kode di bawah ini.
// Jika tidak ingin mengubah displayRiwayat asli, Anda bisa override dengan menempelkan fungsi displayRiwayat yang sudah dimodifikasi.
// Berikut adalah fungsi displayRiwayat yang sudah diperbarui (copy paste untuk mengganti yang lama):
window.displayRiwayat = function(searchTerm = '') {
  let filtered = [...transactions].reverse();
  if (searchTerm) {
    const term = searchTerm.toLowerCase();
    filtered = filtered.filter(t =>
  ...
  container.innerHTML = filtered.map(t => `
    <div class="riwayat-item ${t.voided ? 'voided-transaction' : ''}" style="${t.voided ? 'opacity:0.6; background:#ffe0e0;' : ''}">
      ...
      <div>Items: ${t.items.map(i => `${i.nama} x${i.qty}`).join(', ')}</div>
      <div>
        ${!t.voided ? `<button onclick="showReprintPreview(${JSON.stringify(t).replace(/'/g, "&#39;")})" style="background:#4361ee; color:white; border:none; border-radius:8px; padding:4px 12px; margin-right:8px; cursor:pointer;">🖨️ Cetak Ulang</button>` : ''}
        ${t.voided ? '<span style="background:#dc2626; color:white; padding:2px 8px; border-radius:12px; font-size:12px;">✓ VOID</span>' : (hasRole(['admin','owner']) ? `<button onclick="voidTransaction(${t.id})" class="void-btn" style="background:#ef4444; color:white; border:none; padding:4px 12px; border-radius:8px; cursor:pointer;">Void</button>` : '')}
      </div>
    </div>
  `).join('');
};
...
window.showReprintPreview = showReprintPreview;